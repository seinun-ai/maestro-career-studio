...
| 8 | Health report (base target) → Apply suggestion; punctuation batch; number-questions batch with ≥ 2 rows; Demonstrate skill | note once per action (ONE for the batch — and for the batch it appears AFTER the success toast, by design: it is emitted in `finally`), then the success toast |
| 8b | Number-questions batch, then edit the resume in another tab before Apply all (the 409 content-changed path) | the error toast AND the single note (the `finally` emits it) |
...
| 17 | 7 (planned) | Task 4 covered POST `/render`, PUT and PATCH | Task 7 will ALSO pass `render_note=getattr(row, "render_note", None)` at the create-side return sites of `routers/base_resumes.py` (`POST ""`, `/duplicate`, `/import`, `/from-kb`) with one test | Task 4's re-review: on a TeX-less desktop the user's FIRST action is import or create, and those re-render too; `duplicate` was probed returning `render_note: null`. |
...
| 23 | 5b | Step 0: make the harshibar joiner string raw | Backslashes doubled instead (`ec3812d3`); commit subject says so | Jinja's expression lexer has no `r'…'` prefix (`TemplateSyntaxError` on 3.1.6). Rendered TeX byte-identical. |
| 24 | 5b | `SUPERSEDED_SEED_DIGESTS` for `carlito_dense` and `harshibar` only | As planned; six versions pinned (`e81696be`, `c4b40be0`, `c27b05d4` on `main`, per template): v1/v2 equal the legacy chain's `c84a19d2e7f0`/`0bc45a1bf6d6` pins, v3 verified against `git show c27b05d4` (it is the version those migrations resynced TO, and is byte-identical at `main`'s tip) | `resume.tex.j2` and `xcharter_serif.tex.j2` used `latex_escape_url` from their first commit, so no old version exists to recognise. |
| 25 | 11/12 (planned) | — | Task 11 updates `docs/entities/others.md:113-114` (credits migration `c84a19d2e7f0` for seed resync; the live mechanism is now seed-time `SUPERSEDED_SEED_DIGESTS`) and adds a README to `tests/fixtures/templates_superseded/` like the precedent dir; Task 12 adds the §12 gotcha "seeded rows copy source only on insert; the legacy import lands rows AFTER migrations" | Flagged by Task 5b's implementer; outside its file list. |
| 26 | 6 | Parity test compares word sets of the two PDFs' text | `_words` also strips `(cid:NNN)` tokens (`8b1f5e5e`) | pdfplumber emits the LaTeX header's `\faLinkedin` icon as `(cid:239)` (no ToUnicode entry), which split into `cid`/`239`. One extraction-artifact class, nothing else. |
| 27 | 6 | — | Found, not fixed in `8b1f5e5e`: `_header.tex.j2` emits a bare `\\` when `contact.location` is absent, so the LaTeX cover letter cannot compile for such a contact (pre-existing; surfaced because the router test's contact has no location and now renders on Typst, which guards it) | Fix goes into Task 6's review round (one Jinja guard in the partial, both including templates covered by the existing href tests plus a compile test without location). |
| 28 | 6 | Typst paragraphs are `#p` | Recorded, not changed: a single `\n` inside a paragraph is a space in LaTeX and a line break in Typst | Word-set parity cannot see layout. Decision in Task 6's review round: Typst mirrors LaTeX (`#p.replace("\n", " ")`) so the two engines agree; whether a sign-off SHOULD break is a product question for both engines, not this branch. |
| 29 | 6 | Task 6 touched only the cover-letter files | Also carried three Task 3 quality follow-ups (`8b1f5e5e`): `render_document` re-raises `TemplateMissingExtraSectionsError` with the note prepended after a substitution; the fallback log line carries the probe's reason; `resolve_render_template` annotated under `TYPE_CHECKING` | Task 3's quality reviewer approved with "fold into the task that reopens `pdf_render.py`" (its minors 2, 3, 5). |
| 30 | 6 | Fix round | `_header.tex.j2` guards the location line (same line, byte-identical TeX when present) with compile tests over resume + cover letter × missing/empty location; `cover_letter_typst_inputs` collapses intra-paragraph whitespace so Typst mirrors LaTeX; the resolver probes pdflatex once | Quality review: the bare `\\` was a live 500 on the default LaTeX resume for any contact without a location (pre-existing, disk-loaded partial so it reaches every install); the `\n` divergence would make a substituted letter look different from the one the user had; a broken `pdflatex --version` was spawned twice per fallback render. |
| 31 | 10 (planned) | Row 16 names two frontend render call sites | Task 10 will ALSO surface `render_note` on the QA tab's cover-letter render (`frontend/components/qa-tab.tsx` posts to `/api/qa/{id}/render`) | Third consumer; found in Task 6's quality review. |
| 32 | 6 | Guard the location line with a same-line `((* if … *))…((* endif *))` | Tags at column 0 with the indent inside the body (`cf40c2b5`) | `_environment()` has `trim_blocks=True, lstrip_blocks=True`: the same-line form strips the indent before `((* if` and eats the newline after `endif`, so a contact WITH a location was no longer byte-identical. The column-0 form is identical with a location and emits nothing without one. |
...
| 46 | 10c | "If a version summary, add the field there" | New `ResumeVersionRestoreResult(ResumeVersionSummary)` for the restore route only (`5015c5c5`); frontend `ResumeVersion & RenderNoted` | `ResumeVersionSummary` also answers list/get/label routes; an always-null field on every history row would be noise. |
| 47 | 10c | Three routes | Four: `POST /api/kb/port/adapt/apply` renders through the same `_persist_port` and returns the same `KBPortResponse`; one `notifyRenderNote` in the shared `finishPort` covers both | Leaving it out would have been a fourth silent route. |
| 48 | 10c | — | `tests/test_base_resumes_router.py:340`'s exact-equality assertion on the port-project body now includes `"render_note": None` | The response gained a field. |
| 49 | 10c | Fix round | The chat change-card's Revert (a second version-restore caller) surfaces the note; a test for `adapt/apply`; the onboarding bulk import (`POST /api/kb/import`, service `kb_import.import_resumes`, `ImportedBase`/`ImportedBaseRead`) carries and surfaces the note, once per import batch (`5c08642f`) | Spec review: the revert path was missed (the plan's own grep would have found it); `adapt/apply` was code-only; the bulk import is the first action a TeX-less desktop user takes. |
...
| 50 | 10d (new) | Design §2.2: no ready Typst template on a TeX-less host is a 400, never a 500 | TRUE of `POST /api/kb/port`, FALSE of `POST /{slug}/port-project` and `POST /resume-versions/{kind}/{key}/{n}/restore`: both called `render_base_resume` unwrapped, so both answered 500 over an ALREADY-COMMITTED write with no `render_error` persisted (a silently stale PDF); new Task 10d | Code-quality review of Task 10c; reproduced red before the fix. |
...
|---|---|---|
| G1 suite / ruff / check_system_md / check_mcpb_bundle / slop (backend, frontend) | PASS (2026-09-20) | `pytest tests/ mcp_server/tests/ -q` → **4378 passed, 2 skipped** (merge base was 4280/2: +98 tests). `ruff check .` clean. `check_system_md: OK — 998/1000`. `check_mcpb_bundle: OK`. Slop ratchet: three real frontend duplication regressions found by diffing jscpd reports against the merge base and FIXED (`applyResumeEdits` shared by 6 sites, `toastRewriteError` reused at 3, `notifyRenderOutcome` shared by 4), plus one backend pair (`_port_response`); frontend ratcheted DOWN (duplicated_lines 537→518, clones 45→43), backend duplication 423→418. Re-baselined with a reason per metric (`2a427e7c`): `complexity_hotspots` 417→424 is growth — worst-offender cc unchanged at 42, density DOWN 3.34→3.32 per KLOC, and of the 5 new source hotspots three crossed only the length/param lines; `error_masking` 9→10 is one deliberate `except zlib.error: pass` in a test PDF-stream scanner, commented. |
| G1 browser re-check after the dedup refactor | PASS (2026-09-20) | The four paths verified in the G2 browser pass use `notifyRenderNote` at sites the refactor did not touch; `notifyRenderOutcome`/`applyResumeEdits` landed only at the ten sites that were never browser-exercised (health findings, chat cards, KB port/import, version restore — all backend-test pinned, tsc/lint/build clean). Re-ran gallery Duplicate after the hot reload: note still fires. |
| G2 TeX-less host: render + cover letter through Typst with `render_note`; setup status; gallery | PASS (2026-09-20) | Worktree uvicorn on 8765, `MAESTRO_CS_PDFLATEX=/nonexistent`, scratchpad dirs. `setup/status.engines`: pdflatex `available False` with the override reason, typst `0.15.0 True`; `template.detail.default_engine_available False`. `/api/templates`: every LaTeX seed `draft` / `engine_available False` / `last_error "requires TeX (pdflatex not found)"`; both Typst seeds `ready`. `POST /api/base-resumes/example/render` → `typst` / `typst-classic` / `template_fallback False` / note "…rendered with Typst Classic instead of Classic." / PDF on disk. Application render and `POST /api/qa/{id}/render` (fixture job+application+entry inserted via a scratch script) → same note, PDFs on disk. |
| G2 browser pass (Task 10/10b/10c toasts, badges, checklist) | PASS (2026-09-20) | `next dev` on 3100 proxying 8765. `/templates`: amber `requires TeX` on the three LaTeX cards + the reason line; Typst cards with thumbnails. `/templates/default`: header badge next to `latex`; preview panel shows the reason. `/profile`: pill strip WITHOUT a PDF-engines pill (filter works). `/applications` getting-started card: "PDF engines — Typst ready · TeX not found…". Toasts: application Regenerate PDF → note + "PDF generated"; Q&A Render PDF → note + "Cover letter PDF ready"; gallery Duplicate → note survives the push to the new editor; editor Save → note + "Saved. PDF re-rendered.". Not exercised: health-report applies, chat cards, KB port/import dialogs, version restore (all share `notifyRenderNote`; pinned by the backend tests). |
| G3 Docker image: both engines available, `render_note` null, LaTeX output text-identical | PASS (2026-09-20) | `docker build` from the worktree → `maestro-cs-backend-g3`, run standalone on 8766 with a scratch data volume. `engines.pdflatex`: `True`, `pdfTeX 3.141592653-2.6-1.40.29 (TeX Live 2026)`, `/opt/texlive/texbin/pdflatex`; typst `0.15.0`; `default_engine_available True`; all six seeds `ready` / `engine_available True`. Created a base resume from `example.json` and rendered: `latex` / `default` / `render_note None` / PDF on disk. Text identity: the LaTeX templates' TeX output is unchanged for a contact with a location (byte-identical, verified in Tasks 5/6 reviews); the href fix changes link targets only. |
