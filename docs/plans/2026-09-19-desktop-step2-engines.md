...
| 8 | 2 | Files: `pdf_render.py`, `engines.py`, two test files | Also `tests/test_pdf_render.py:45` (`argv[0] == "pdflatex"` → `.endswith`) and `tests/test_security_boundaries.py` (the hardened-env tripwire counted `>= 2` spawns; now `== 1` plus a check that both call sites route through `_run_pdflatex`) | The single runner the task mandated changes the code shape both tests pinned. The tripwire's intent (every spawn carries `env=`) is kept; the spec reviewer was asked to confirm the new guard still fails on each regression it was built for. |
