...
| 49 | 10c | Fix round | The chat change-card's Revert (a second version-restore caller) surfaces the note; a test for `adapt/apply`; the onboarding bulk import (`POST /api/kb/import-resumes`, `ImportedBase`) carries and surfaces the note | Spec review: the revert path was missed (the plan's own grep would have found it); `adapt/apply` was code-only; the bulk import is the first action a TeX-less desktop user takes. |
