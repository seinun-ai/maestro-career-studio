...
| 24 | 5b | `SUPERSEDED_SEED_DIGESTS` for `carlito_dense` and `harshibar` only | As planned; six versions pinned (`e81696be`, `c4b40be0`, `c27b05d4` on `main`, per template), each equal to a legacy-chain migration pin | `resume.tex.j2` and `xcharter_serif.tex.j2` used `latex_escape_url` from their first commit, so no old version exists to recognise. |
