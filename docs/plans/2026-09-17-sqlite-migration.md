...
| 8 slop ratchet | FAIL → fixed in follow-up | `orphan_loc 0→145` (`app.tools.backup_db` not an entry point), `duplicated_lines 420→570`/`clone_count 45→47` (the one-release `legacy_postgres/alembic.ini` copy), `error_masking 9→10` (`backup_db._silence_stdout`), `complexity_hotspots 411→417` (growth; re-baselined with reason) |
