...
| — (housekeeping from the Task 7 review) | `tests/conftest.py::_test_engine` does not declare `_test_database_ready`; the parity tests now take `_test_engine` directly and rely on autouse ordering | add the fixture parameter so the dependency is explicit |
| `AssertionError: missing backend/migrations/prompt_defaults.lock.json` (23 errors in `tests/test_prompt_sync_guard.py`) | Task 6 moved the file into `legacy_postgres/migrations/`; it is a TEST pin over the seeded prompt defaults, not migration data, and the legacy box is deleted in N+1 | `git mv legacy_postgres/migrations/prompt_defaults.lock.json migrations/prompt_defaults.lock.json` (the test's `LOCK_PATH` is right; the file was in the wrong box) |
| `expected the … resync migration` (12 failures in `tests/test_template_section_order_resync.py`, `tests/test_kb_capture_resync.py`, `tests/test_template_date_resync.py`) | they glob `migrations/versions/*_resync_*.py`, which now lives under `legacy_postgres/` | point the globs at `legacy_postgres/migrations/versions/` (the chain still ships and the importer runs it this release) and add the three files to the §13 `postgres-to-sqlite` removal list |
| `'str' object has no attribute 'hex'` from APP code (`app/services/proposals.py:66 create_proposal`) | a string id reaches a `UUIDType` column | convert at the boundary (`uuid.UUID(value)` where the string enters); pin with a test |
| `test_duplicate_identity_keys_always_match_the_same_entity` lands on the newer duplicate entity | a query with no deterministic `ORDER BY` relied on Postgres heap order | add the `order_by` the behaviour promises (older entity wins: `created_at`, then id) in `services/kb_consolidation.py`; do not sort in the test |
...
6. **Bind-mount WAL smoke** (macOS Docker Desktop): 200 concurrent `POST /api/autofill/telemetry` requests with a VALID `TelemetryBatch` body (`page_host`, `action`, `observations[]`; a bare `[]` is a 422 rejected before any DB access and tests nothing), then the grep above prints nothing and `/health` still answers. Caution: a copy of a real database carries the stored API key in `settings`; null that row (or override the key) before exercising any LLM-triggering route on a scratch stack.
...
| 6 | Alembic renders a TypeDecorator as `sa.<Name>()` | Alembic 1.18.3 renders it fully qualified, `app.models.types.UTCDateTime()`, equally unimportable in the revision | 46 occurrences replaced with `sa.DateTime()` by hand; baseline `871d0425b64c`, 27 tables | — |
| 6 | `alembic check` proves the baseline | it runs with `compare_server_default=False`; a scratch `compare_metadata` with it ON found only Alembic's known empty-string TEXT false positives | recorded; server defaults were reviewed by eye | — |
...
| 8 | trailer `Claude Fable 5.1` | the Task 8 implementer ran on Opus and signed `Co-Authored-By: Claude Opus 5 (1M context)`; accurate attribution beats a uniform trailer | left as is; later Opus-run commits may do the same | — |
| 8 | average Python floats, `round(…, 1)` at the end | Postgres averaged `numeric` exactly, so `.x5` ties rounded the other way in ~1.3% of buckets (56.3, 97.6 → 77.0 vs 76.9) | keep the `Decimal` values SQLAlchemy returns and divide exactly; pinned by a two-score test | deterministic scores |
| 8 | ordering reproduced by hand-rolled `sorted(...)` | no test pinned the order the two chart endpoints promise | ordering assertions added to both tests | correctness |
| 8 | — | `role_mix_over_time` and `ats_over_time` now transfer every row instead of a grouped result; bounded only by single-user scale (design §2.4 accepts this) | note for a future retention story; no change | — |
| 9 | `services/proposals.py:66` binds a string id | the only app-side `.hex` came from a test passing a JSON str straight into `create_proposal`; the public route parses `UUID`; a sweep found every boundary already converting | tests wrap ids in `UUID(...)`; a route test pins that string ids still cross into `UUIDType` columns; no app change | — |
...
| 11 | `normalize_value` formats Decimals with `format(v, "f")` | unscaled `Numeric` (`jobs.salary_min/max`) comes back `Decimal('120000')` from psycopg and `Decimal('120000.0000000000')` from SQLite; same value, different hash | trailing fractional zeros stripped (a value change still mismatches); pinned by the real-Postgres test with salary columns | no data loss |
| 11 | plan's raw legacy INSERT | legacy `jobs` never had `updated_at`; `AtsScore` needs `engine_version` | test rows adjusted | — |
...
| 14 | design §4: "New keys: none" | `SQLITE_JOURNAL_MODE` was never forwarded by compose, so the documented escape hatch could not be set from `.env` | `SQLITE_JOURNAL_MODE: ${SQLITE_JOURNAL_MODE:-}` in the backend environment and a commented `.env.example` entry; `data/` named in the APP_UID note; design §4 rewritten | correctness |
| 15 | commit message `update.sh: SQLite-aware backup…` | implementer used `fix(update): back up SQLite, not Postgres, once the import has run` | accepted; the header comment was rewritten within `usage()`'s fixed `sed -n '2,18p'` range | — |
| 15 | `legacy_postgres_is_live` = marker absent AND volume present | an install that skipped the import (unset `LEGACY_DATABASE_URL`) would `pg_dump` a dead Postgres on every update and never snapshot SQLite; the streamed snapshot was created with the caller's umask (0644); a zero-byte gzip passed the empty-backup guard | predicate also requires an empty SQLite file; `umask 077` + `chmod 600`; magic-byte checks on both branches; restore recipe quoted and ends with `up -d`; the fail-closed note gated on a pending import | no data loss / security boundary |
| 15 | coordinator: `legacy_postgres_is_live` should also require an EMPTY SQLite file | a failed-closed first boot leaves a schema-only, row-empty file that is non-empty in bytes while Postgres still holds everything; a size check would have snapshotted the empty file and called it the rollback | executor deviation accepted: the predicate stays (marker absent AND volume present); when legacy is live AND the file has bytes, BOTH a pg_dump and a SQLite snapshot are taken and the restore note says which to use; `do_check` names the ambiguous state | no data loss |
...
| 16 | the legacy import job runs the marked test with `-q` | the test `pytest.skip`s when the env var is unset, so a broken env assignment turns the job into a permanent green no-op (exit 0, one `s`) | `-rs`, pip cache shared with the suite job, and the test FAILS instead of skipping when `CI` is set | correctness |
| 17 | SYSTEM.md edits fit under the 1000-line cap | the file sat at exactly 1000/1000; ~45 lines of new content needed room | the touched sections (§9, §11, §12, §13 prose) were reflowed to a wider column, word-stream verified lossless; no re-baseline; gate OK at 997/1000. Also fixed beyond the spec: §7's prompt-reset sentence and §13's `latex-render-path`/`health-rewrite-cache` notes referenced revisions now in the legacy box | caps are earned |
| 18 | — | the boot error text says "unset LEGACY_DATABASE_URL", which compose cannot do; `backend/mcp_server/README.md` diagram and `docs/entities/others.md` still said Postgres; README's "three images ~1 GB" split into two app images ~850 MB plus a one-last-time Postgres pull | docs written to the compose-file instruction; the three stale spots and the boot message go to the Task 19 sweep | correctness |
| 19 | gate 5 "without an API key the enrichment call fails fast" | the imported live database carries the stored OpenAI key, so the enriched session made a real ~39 s LLM call (a few cents) | gate text amended; scratch copy of the live DB deleted after the gates | security boundary |
| 19 | gate hammer body `[]` | a 422 before any DB access | gate text amended to a valid `TelemetryBatch`; gates 5/6 re-run with valid bodies | correctness |
| 18 | docs review: three medium inaccuracies in the update-window story | the backend has no healthcheck; the manual backup lost its Postgres branch; the importing update produces only a `.sql.gz`, which the rollback recipe no longer explained | fixed in `d6debe9a`; also: the restore note prints after every backup AND on a failed health wait, so the docs say that | correctness |
| 19 | slop ratchet baseline | `backup_db` not an entry point (145 orphan LOC); the one-release `legacy_postgres/alembic.ini` copy counted as a clone; `_silence_stdout`'s except-pass; hotspot count +6 from growth | entry points registered, `**/legacy_postgres/**` duplication-ignored with a reason, the guard restructured, hotspots re-baselined with the reason in the commit message (`d5772e23`); surface: backend | caps are earned |
| final | whole-branch review | clean-slate recipe deleted the marker while the volume remained (the next boot would re-import the old data); `update.sh`'s SQLite backup ran on `.env`'s `IMAGE_TAG` before the release pin (a stale `:latest` lacks `backup_db`); CHANGELOG missed the `DATABASE_URL` refusal, the alembic squash and the `TEST_DATABASE_URL` rule; importer wrote JSON `'null'` text for SQL NULL; a malformed URL could echo its password | all fixed in the final two commits; design §7 records the decisions as taken pending objection | no data loss / security boundary |
...
|---|---|---|
| 1 suite | PASS | `4277 passed, 2 skipped, 15 warnings in 223.22s` at `acd1dce1` |
| 2 lint | PASS | `ruff check .` → All checks passed |
| 3 docs gates | PASS | `check_system_md: OK — 998/1000`; `check_mcpb_bundle: OK — 2 files match`; `node --test mcpb/tests` 4 pass |
| 4 monotonicity | PASS | `2015 pairs checked, 0 violation(s)` on the SQLite import |
| 5 concurrency | PASS | uvicorn on :18011 over a copy of the imported DB: `POST /api/tailoring-sessions` (enrich=true, a real LLM call, 39 s, 200) concurrent with `POST /api/ats-scores` (200, 5 s); 200 concurrent telemetry POSTs with VALID bodies (`TelemetryBatch` object, not `[]`) + 10 interleaved `GET /api/jobs` → all 2xx; log: `database is locked: 0`, `500s: 0` |
| 6 bind-mount WAL | PASS | backend image built from the worktree, run on Docker Desktop with `-v <scratch>/data:/app/data`: same hammer, `database is locked: 0`, `500s: 0`, in-container `pragma journal_mode` = `wal` before and after, `-wal` grew to 4.17 MB on the VirtioFS mount; `SQLITE_JOURNAL_MODE=DELETE` not needed |
| 7 update rehearsal | manual, not run | needs a scratch clone with real Postgres data on a second machine (owner) |
| 8 slop ratchet | PASS after `d5772e23` | `orphan_loc 0→145` (`app.tools.backup_db` not an entry point), `duplicated_lines 420→570`/`clone_count 45→47` (the one-release `legacy_postgres/alembic.ini` copy), `error_masking 9→10` (`backup_db._silence_stdout`), `complexity_hotspots 411→417` (growth; re-baselined with reason) |
