...
| fix pass (plan defect, D8 §8 `stages/fill.js:664-665`) | "Uses your saved answers, then asks the AI for the rest. Your personal details never go to the AI." | "… The AI sees your saved answers and career history." `/api/autofill/choose` puts the autofill profile, the career history and the persona in the prompt (`services/autofill_choose.py`, `prompts/autofill_choose.txt` `${profile}`). Both old wordings are refused by `test_no_panel_sentence_promises_the_ai_never_sees_your_details` | Honesty survives every cut: the privacy promise was false |
| fix pass (§6 violation, planner decision) | Not in D | `/choose` sent the profile UNFILTERED, so diversity (EEO) answers reached the model with consent off. One gate, `eeo_consent.withhold_unconsented` (fails closed), now used by GET `/context` and by the `/choose` prompt (`eeo_consent.disclosable_profile`). Own commit `21c546d7`; `inv-eeo-standing-consent` moved from the unpinned list to three pins in `.system_md_enforcement.json` | inv-eeo-standing-consent: which path asks never decides what is disclosed |
| fix pass (plan defect, D8 §8.3 `actions/resume.js:81`) | "Couldn't tailor your resume. Open the job in Maestro CS to see why." | "Couldn't tailor your resume. Use Tailor in Maestro CS to see what's in the way." The job page alone shows no reason; the link opens the Fit tab, which lists a gap analysis in progress, and starting one there shows the must-fix sentence (`ats-score-panel.tsx` `createSession.onError`). Checked against the backend: a real 409 is "A gap analysis for this job and resume is in progress…" | Every sentence tells the truth |
| fix pass (planner decision I7) | `failureNote` had no 502 branch | 502 (LLMProviderError via `app.main`) → "{what} Check your AI key in Maestro CS under Settings › AI & models." on every action, ahead of a call site's `answered`; the generic "{what} Try again." branch pinned (500, 409) | The step a user can take |
| fix pass (plan defect, D8 "Apostrophes: keep each file's existing character") | Curly ’ in `pause.js`, `qna.js`, `stages/fill.js`, straight elsewhere | Straight everywhere in the panel, per D §1 ("a string literal writes `'`"), which the web app (518 straight, 6 curly) and every server sentence the panel shows follow. Pinned: `test_the_panel_writes_one_apostrophe`. The reviewer's note wrote "(’)"; D §1 is the binding rule, so straight | One word (and one character) per thing |
| fix pass (found in Chrome) | "No saved answers yet…" to be shown | It never reached the note (the runner swallows the rule pass's throw); it now leads the run's note. Found in Chrome for Testing: over an empty profile the note then read "… Fill finished. Review before you submit." on a run that wrote nothing, so "Fill finished" follows only a run that wrote something (`fillNote`, pinned both ways) | Never claim what did not happen |
| fix pass (plan defect, D9.6 `llm.py:232, :304`) | "The AI model didn't answer ({short provider reason})", implemented as "error 429: insufficient_quota" | The code in words ("your key was refused", "your account is out of credit", "that model wasn't found", "too many requests"; else "error {status}"); the raw code stays in `provider_detail`. An underscore makes `isPlainSentence` hide the whole message | Plain words; no hidden sentences |
| fix pass (plan defect, D9.6 `llm_capabilities.py:238-243`, `model_settings.py:342-344`) | "{model} can't use tools ({reason}). …", "{id} is your Fast, Smart or Assistant model. …" | "The model {model} can't use tools. …" (the probe's reason to the log), "The model {id} is your …": a sentence starting with a lower-case id, or carrying a provider's text, is hidden by `isPlainSentence` | Same |
| fix pass (plan defect, D9.6 `attachment_extract.py:153-160`; lane scope rows `base_resumes.py`, `career_kb.py:816`) | "No text could be read in {f}.", "{f} isn't a readable image.", "{file} isn't a resume in the Maestro CS JSON format.", "Couldn't read {name}. Try again." | No file name in the sentence ("This file isn't …", "No text could be read in this file.", "Couldn't read this file. Try again."): the dialogs print the name beside the reason, and a name like `my_cv.pdf` hides the whole sentence. `career_kb.py` `/consolidate`'s read error and the import 422 ("No resumes could be imported. {reason}", no name) follow. One JSON sentence on both import paths (`kb_import.parse_resume_json`) | Same; one sentence for one failure |
| fix pass (plan defect, D9.4 `layers.py:669`) | "Your dates show about {y}." (whole number) | Could read "The job asks for 5+ years. Your dates show about 5." (4.6 rounds to 5; the warning fires below 4.75). Now one decimal when the whole number would reach what is asked, "less than a year" below 1 | Never contradict itself |
| fix pass (plan defect, D9.2 `resume_lint.py:389-390`) | "Your summary and newest role are weak, …" | False for an early-career resume, whose hot zone is its first PROJECT (`health_zones.hot_locations`): "Your summary and first project are weak, …" there. The C1 gate finding keeps its PRE-lane id (`_id_key_c1`, e_hot to two places) | True words, stable ids |
| fix pass (plan defect, D9.6 `services/proposals.py:94`) | "This proposal is already {status label} and can't be changed." | "This proposal's status is {chip label}, so it can't be changed that way." with the Agent inbox chip words (Proposed, Needs you, Queued, Approved, Applied, Check if sent, Skipped, Expired), `STATUS_CHIP_WORDS` pinned equal to `PROPOSAL_STATUS_CHIP` | One word per thing |
| fix pass (plan defect, D9.6 `pdf_render.py:555-558`) | "This template needs a tool that isn't installed. Pick another template." | Raised only when NO template here can make the PDF, so "pick another" was false: "This template needs TeX, a tool that isn't installed on this computer. Install TeX, or add a Typst template, which doesn't need it." The fallback note stays free of engine names (pinned) | Say what works |
| fix pass (plan defect, D9.4 `engine.py:13`) | "Your resume shows only {m} of this job's {n} skills ({p}%)." | For zero: "Your resume shows none of this job's {n} skills." | Plain words |
| fix pass (I6) | Gate labels only in the gate constructors | `health_gates.GATE_LABELS` by id: `make_gate` stamps it, `_must_fix_message` names by it, and `routers/resume_lint._read` re-stamps stored reports (`with_current_labels`, a copy), so reports computed before this lane show today's labels and the frontend needs no change | One table |
| fix pass (I5) | Parity test skips without node | `tests/node_ts.py` (shared by `test_agent_names.py` and `test_server_words.py`) fails when `CI` is set; the backend CI job sets up node 24 | A parity check nobody runs is not one |
...
| fix pass | pins seen failing first | privacy 2/3 (consent-on already passed), sw PDF status, the Companion sentences (C1, 502, quick tailor, no saved answers, apostrophes) and every rewritten server pin failed on the old code before the change |
| fix pass | mutations | privacy 5/5; Companion 12/12 (E1–E10, plus the ban test and the Quick tailor 502 alone); server 35/35 (S26 survived first — the read path editing the stored dict in place — and was killed by `test_relabelling_a_stored_report_leaves_the_stored_dict_alone`; S35, the parity test skipping in CI, is killed with `CI=1` and no node on PATH: 2 failed vs 2 skipped) |
| fix pass | full backend | `pytest tests/ mcp_server/tests/ -q`: 5381 passed, 3 skipped (was 5336 passed, 3 skipped); `ruff check .` clean; all `test_extension_*.py` 970 passed; `test_extension_*.py` + `test_frontend_*.py` 1918 passed (after the Companion commit) |
| fix pass | frontend | `tsc --noEmit` clean; lint 0 errors, 2 warnings; `node --test lib/*.test.ts` 230; `npm run build` OK (run last) |
| fix pass | slop, clean `git archive HEAD` at `554f9279` | frontend dup 437 / 36; backend `complexity_hotspots` 424, dup 404 / 44; extension 0 hotspots, 0 clones, 7 allowlisted (unchanged); `check frontend|backend|extension` OK; `check_system_md.py` OK (1000/1000) |
| fix pass | real Chrome | Chrome for Testing, the unpacked extension, the REAL side panel over CDP, backend 8874 with no key, made-up posting on 8875, light 360 and 400, dark 320: Fill mode sentence "… The AI sees your saved answers and career history."; Quick tailor 502 "Couldn't tailor your resume. Check your AI key in Maestro CS under Settings › AI & models."; Quick tailor 409 (a gap analysis in progress) "Couldn't tailor your resume. Use Tailor in Maestro CS to see what's in the way."; an empty profile's fill "No saved answers yet. Add them in Maestro CS under Profile › Autofill." (and no "Fill finished" after it: the defect found here); no horizontal overflow |
...
- §6 `inv-eeo-standing-consent` (SYSTEM.md:383-392): "`GET /api/autofill/context` strips `profile.eeo` unless `enabled` and fails CLOSED …" → the strip is ONE gate, `eeo_consent.withhold_unconsented`, used by GET `/context` AND the `/api/autofill/choose` prompt (a model provider is a recipient too); now pinned (`.system_md_enforcement.json`, three pins; dropped from `unpinned`). §12 (≤3 lines): "2026-09-24: `/choose` put the unfiltered profile in the prompt, so EEO answers reached the model with consent off → every reader that hands the profile outward goes through `withhold_unconsented`."
- §7 Guided fill (SYSTEM.md:623): "identity data never reaches `/choose`" is true only of the retryables' VALUES (the fields list); the prompt carries the saved answers (consent-gated EEO) and the career history. Reword to say both.
- §7 panel: `failureNote` 502 → the AI-key step; a no-profile run leads its note with "No saved answers yet …"; `attach_pdf`'s error carries its status like `api()`'s.
- §7 or §12 (health): `health_gates.GATE_LABELS` is the one table of gate words, re-stamped on READ (`with_current_labels`), so a gate relabel needs no re-run and no frontend map.
- §9 (dev & test): the backend CI job has node 24 for the TypeScript parity tests (`tests/node_ts.py`), which fail rather than skip under `CI`.
- Task 24 docs (not SYSTEM.md; re-verified at `554f9279`): `extension/README.md:12` ("as-is"), `:113` ("Start fill"); `extension/INTERNALS.md:33` ("add job", "start fill"), `:66` ("Score all bases", "Start fill"), `:71` ("Skipped — using base as-is"), `:101` ("Add job"), `:121` ("Score all bases"), `:128` ("Use base as-is"), `:133` ("Custom in Studio ↗"), `:136` ("Use base as-is"), `:139` ("Add job"), `:152` ("Start fill"), `:155-156` ("Rules only", "Rules + AI assist"), `:158-160` (add: the AI pass's prompt carries the saved answers and career history; only the retryables' values stay out), `:226`, `:469` ("Start fill").
...
- RESOLVED in the fix pass: the read path re-stamps stored gate labels from `GATE_LABELS` (`health_gates.with_current_labels`), so reports computed before this lane show today's labels with no frontend map.
- Merge with lanes 7 and 8 FIRST: the gate labels (`GATE_LABELS`), the coverage sentence and the gap page's flag words are read by their files.
- Lane 7's duplicate coverage sentence: `frontend/components/ats-score-panel.tsx:~144` and the gap page `frontend/app/jobs/[id]/tailor/[sessionId]/page.tsx:~752` render a sub-line "We recognized only {m} of the job's {n} skills ({p}%)." under the server's `coverage_warning`, which now says the same ("Your resume shows only …" / "… none of …"). Drop both sub-lines at merge.
- Lane 9's `frontend/components/settings/models-section.tsx:~130`: a double period where a sentence is composed around a server detail that already ends in "."; fix at merge.
