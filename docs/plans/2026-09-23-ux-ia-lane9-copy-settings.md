...
| 21 fix (plan defect, D6.4) | Consent box copy "Terms and acknowledgement boxes only." / "This covers only an application's own terms and acknowledgement boxes." | Lists every family `consent_forms` unlocks (`extension/shared/policy.js` CONSENT_FORMS, `schemas/eeo_consent.py`): "…agreement boxes: terms, acknowledgements, certifications, arbitration and waivers." / "Terms, certifications, arbitration and waiver boxes." "waiver" joins the vocabulary `_ALLOWED` (a legal waiver, not the health check's "waive") | Honesty nuance survives: the consent named less than it grants |
| 21 fix (plan defect, D6.4) | Switch hint "Uses only your exact answers above." | "below": the permission box renders above the diversity questions | D's wording was wrong against the layout |
| 21 fix (plan defect, D6.4 / D0) | Bare "Companion" in the card description, dialog titles and toasts | "the Companion" in every sentence; bare only in the switch labels (planner decision 20) | Glossary |
| 21 fix (plan defect, D7.3) | Empty state "You can undo any edit." | "You can undo any resume edit." (a template change makes no version) | Honesty |
| 21 fix (plan defect, D6.1 :169) | tailoring_skill "Tailoring skills / How skills are added while tailoring."; kb_entity_resolve "How a new bullet finds its item."; gap_enrichment "How gaps get suggested answers." | "Tailoring rules / The honesty and wording rules for every tailoring edit."; "How roles and projects on your resumes are matched to career history items." (not the reviewer's "entries": banned word; the prompt resolves experience and project groups); "Suggested wording and a question for you on each gap. It never claims a skill your resume doesn't show." | Words say what the prompt file does |
| 21 fix (plan defect, D7.2 dropzone) | "Use PDF, Word, Markdown or text." for every picker | `acceptedTypesLabel(accept)` in `lib/upload-accept.ts`, read from the caller's own list (no prop, so the lane-8 callers need no edit): resumes "PDF, Word, Markdown, text, LaTeX or JSON", documents "…or an image (PNG, JPG or WebP)" | A resume picker takes JSON and `.tex` |
| 21 fix (planned "Keep the words in ${…}") | Editor hint quoting `${…}` | "Keep every word that starts with $. The app fills them in." | 18 prompt files use `$name`, 4 use `${name}` |
| 21 fix (first-read "Approve or retire") | "Approving or retiring bullets" | "Approve bullets, or mark them Not used, in your career history. They're told to do this only after you say yes. That's a rule they're given, not a lock." | "retire" is banned (glossary: Not used) |
| 21 fix (first-read agreement box) | Move the agreement-box switch "into its own Companion permissions box" | A `CompanionPermissions` box heads the Autofill card (it covers every form); the diversity switch keeps its box inside Diversity, headed "Permission"; each box shows "You agreed on…" for its own switch | Consent semantics unchanged (one record, two decisions) |
| 21 fix (item 8) | "Read setup status or catch the specific error" | The router resolves the chat client BEFORE the capability check (a keyless model test stores all No, which then answered first with a raw message) and answers 422 `NO_MODEL_KEY`; the page shows any 422 beside the composer with "Open Settings" (`/settings#api-keys`). Not setup status: `model_key` is false with a custom local server, which needs no key | SYSTEM §12 (check in the router, never in `run_turn`); additive |
| 21 fix (item 11) | Hand focus to the next chat after delete | The confirm's `returnFocus` (only once confirmed) picks the successor as the dialog closes; the `leaving` ref and its effect are gone | The effect ran while focus was still in the closing dialog |
| 21 fix (item 14 PDF step) | "hidden or reworded when ready" | Both: "PDF creation is ready." (and "…A few templates need extra software…" without TeX); Getting started drops the row when both engines are there | No internals |
| 21 fix (first-read race "(optional)") | Drop `optional: true` on race | `optional` stays (readiness: `setup_status` counts four diversity answers, pinned by `test_autofill_groups_parity.py`); the Diversity group's labels omit the "(optional)" mark instead | The mark means "not needed for readiness"; the whole group is voluntary |
| 21 fix (browser) | not in the brief | A model test with no key said "{model} can't do Writing, Structured answers, Assistant. Everything else works." and the marks' reasons held the raw server error: now "Couldn't test {name}. Add an API key in Settings › AI & models.", "Everything else works." only when something did, reasons in words | Honesty |
| 21 fix (coordinator) | `errorDetail()` recognises the server's key errors | Missing key (`No … API key`, `*_API_KEY is required`) → "Add an API key in Settings › AI & models."; refused key (`invalid_api_key`, `Incorrect API key`, `API key not valid`, `Error code: 401`) → "Check your API key in Settings › AI & models." Checked before the plain-sentence test so the fix reads the same whatever lane 10's rewritten server words are | Every caller's "Couldn't …" says the fix |
...
| 21 fix | pins seen failing first | 30 new or changed pins failed before the code (plus 2 for the browser-found model-test fix) |
| 21 fix | mutation checks | 57 mutations (script: this session's scratchpad `mutate9.py`) (N1–N57 plus 3 node and 1 backend), each killed by its pin; N56 (the permissions box not rendered) survived once, and the pin now checks it renders first. A first run's extra `test_no_raw_error_text_reaches_the_screen` failures came from `*.bak9` backups beside the source (the ratchet globs `*.ts*`); re-run with backups outside the tree, only the intended pins fail |
| 21 fix | `test_frontend_*.py` | 999 passed |
| 21 fix | full backend | `pytest tests/ mcp_server/tests/ -q`: 5368 passed, 3 skipped (a first run caught `test_autofill_groups_parity` [eeo] when race lost `optional: true`: fixed below); `ruff check .` clean |
| 21 fix | frontend | tsc clean; lint 0 errors, 2 warnings (baseline); node 233 pass; `npm run build` OK (run last) |
| 21 fix | slop (clean `git archive HEAD` of b27f6821, and again with the final race fix overlaid) | frontend duplication 437 lines / 36 clones; backend `complexity_hotspots` 424; `check frontend` and `check backend` OK |
| 21 fix | browser (8873/3273, no keys; light and dark, 1280/768/375) | 277 scripted checks passed (plus a 375 sheet check with the backend down; the race label was checked before its final one-line change, pinned since): no key (text kept, "The Assistant needs an API key…", Open Settings link, focus in the composer, nothing saved), first message (focus never on <body>, your message shown once, status "The Assistant is replying…" then "Reading your resume…"), delete (Cancel first, Cancel keeps Delete, next chat, previous chat, New chat; rail and sheet), backend down ("Couldn't load your chats." + Try again, rail and sheet), consent confirms open on Cancel and Enter grants nothing, every new-tab link carries the cue, the catalog shows no restating id, Find models / model test / persona draft name the key, the import dialog's two pickers name their types, New chat and Save answers double-clicks send one request |
...
- §7 (in-app chat) or §12: `POST /api/chat/sessions/{id}/messages` resolves the model client in the router (a missing key is a 422 "The Assistant needs an API key…", before the capability check) and passes it to `run_turn`.
- `docs/frontend-conventions.md` gained: consent confirms open on Cancel, the key sentence in *Errors*, and the Assistant bullet's composer, history and `NewTabCue` rules.
- Owner check (not changed, per the planner): the price figures in "Which model should I pick?" (`models-section.tsx` `ModelProfileNote`).
- §11 candidate (pre-existing, not fixed): a send in a NEW chat creates the session before the message, so a failed send (no key) leaves an "Untitled chat" in the history.
- §11 candidate (pre-existing, lane 10's `llm_capabilities`): a model tested with no key is stored as tools=No and blocks the Assistant until it is re-tested after a key is added.
...
- Fix pass, other lanes' files (not touched): `components/career/resume-import-dialog.tsx:127` and `components/base-resumes/new-base-resume-dialog.tsx:725` (lane 8) hints "PDF, DOCX, Markdown, text, or the app's own JSON · …" → "PDF, Word, Markdown, text, LaTeX or JSON. Up to 10 files, 10 MB each." (the `·` and "app's own JSON" go); the Career history header's "Add documents" (lane 8, `app/career/page.tsx:76`) and the ATS panel's "Import resumes" (lane 7, `components/ats-score-panel.tsx:395`) open the dialog now titled "Import resumes and documents"; the Applications empty state "Capture a job description…" and its "New application" button (lane 7). The sidebar already reads "Add job" and "Career history" (`app-sidebar.tsx`).
- Fix pass, shared test files: `test_frontend_vocabulary.py` `_ALLOWED` gains ("…/autofill-section.tsx", "waiver"); `test_frontend_query_error_states.py` chat-page row now `const threadFailed = …`; `test_frontend_agent_words.py` rows for the auto-apply hint, "Jobs per search", "The Companion uses these…", the new "They can" item and the honesty sentence; `test_frontend_placeholders.py` key-hint lines; `test_frontend_settings_cards.py` the `showsModelId` line and the Fill button marker. `lib/error-text.ts` (Task 16's) gains the key sentences at the coordinator's request.
- Fix pass, reviewer's `/tmp/maestro-ia-review9/mutate.py`: M3's pattern ("Terms and acknowledgement boxes only…"), M4 ("(WOTC)"), M8, M13/M14 (the effect is gone; the handoff is the confirm's `returnFocus`), M28 and M29 read the old strings; the fix pass's own table (N1–N57) replaces them.
