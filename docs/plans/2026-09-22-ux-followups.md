...
|---|---|---|---|
| 18 | Add §11 items; groom to stay under the cap | Seven §11 entries (26–32), the lanes' queued items merged by kind; the two §12 studio entries merged into one; the tracker filter's option rules moved from §5 to the conventions doc; §9's slop-ratchet bullet and §11 items 2/24/25 compressed; short prose in §1/§3/§4/§8/§10 reflowed to the file's width. The `useSearchParams` Suspense rule stays in the conventions sidebar bullet only; the lane-3 orange hue question is left to the owner | SYSTEM.md was at 998/1000; the contract says extract reference and never split orientation content |
| Lane 3 (review) | Humanized-slug fallback; orange-700 Needs-you; tests as written | Claude fix `8c84efee`: acronym-safe `humanizeSlug`, orange-800 chip (and Submission uncertain at merge, `b1e4250d`), solid rings, fit-distribution legend shows résumé names and caps at the palette, pins rebuilt so 38 mutants fail; lane 2's focus-ring allowlist deleted at merge | "AI Ml Engineer" while the catalog loads (no slugs); 4.37:1 on the page (AA); 11 of 18 reviewer mutants passed the lane's pins |
| Lane 4 (review) | Dialog form owned its fields and its create | Claude fixes `7d508140`, `22337aaf`: the page owns the draft and the one create, so closing the dialog keeps typed text and can't double-create; pins for the page's behaviour; the picker's sample description applied at merge (`f462e2a5`) | Principles: a gesture that could lose typed text keeps it; one referral per submit |
