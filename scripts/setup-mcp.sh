...
if [ "$SKIP_INSTALL" -eq 0 ]; then
  if [ ! -d "$VENV" ]; then
    ...
    [ -n "$py" ] || die "no python3 on PATH; the MCP server needs Python >= 3.12"
    ...
    "$py" -c 'import sys; raise SystemExit(0 if sys.version_info[:2] >= (3, 12) else 1)' \
      || die "python $ver is too old; the MCP server needs >= 3.12 (found at $py)"
...
if [ "$PRINT_ONLY" -eq 1 ]; then
  ...
  warn "the 'claude' CLI is not on PATH — skipping Claude Code registration"
fi
...
────────────────────────────────────────────────────────────────────────
Claude Desktop — merge into the "mcpServers" object of
  ~/Library/Application Support/Claude/claude_desktop_config.json
then fully quit and reopen Claude Desktop (Cmd+Q — a window close is not enough).
