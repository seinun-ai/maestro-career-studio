...
def test_a_button_whose_options_never_match_is_released(tmp_path):
    ...
    assert collected_labels(result) == [label]
    assert result["collected"]["retryables"] == []
    assert result["values"][label] == "Select One"


def test_an_option_mismatch_emits_no_write_telemetry(tmp_path):
    """A released mismatch is a non-attempt, so the frozen wire emits nothing."""
    label = "country select one required | country--country"
    result = run_profile_fill(
        tmp_path,
        fields=[{"label": label, "kind": "listboxButton",
