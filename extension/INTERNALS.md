...
- **Workday's dropdowns are buttons, and they are filled.** Workday renders
  ...
  emitting no telemetry. The writer opens the popup, picks by the same scorer,
  and reads the BUTTON'S OWN TEXT back: a click the page cancelled is reported
  as a snap failure, never as filled. A button already showing an answer is left
